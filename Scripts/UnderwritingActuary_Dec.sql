...
SELECT M.*,Decode(ACARDID,'0739-1-2015-9380',(ADDPREMIUM+REFUNDPREMIUM)*2,(ADDPREMIUM+REFUNDPREMIUM))TOTALPREMIUM FROM (SELECT  JV.REF_NO,JV.REFDATE,IMP.ACARDID,
...
AND REFDATE BETWEEN TO_DATE('01/11/2016','DD/MM/RRRR')
AND TO_DATE('30/11/2016','DD/MM/RRRR') AND JV.REF_CODE = DTL.POLICYFINANCEPOSTINGCODE  AND DTL.MEMBERPOLICYCODE = IMP.MEMBERPOLICYCODE
...
WHERE  JV.REF_CODE <> 100000000000035132 AND  JV.JVPOSTINGCODE NOT IN(SELECT * FROM IM_REVERSE_DOC_06) AND JVPREMIUMTYPE = 0 
AND REFDATE BETWEEN TO_DATE('01/11/2016','DD/MM/RRRR')
AND TO_DATE('30/11/2016','DD/MM/RRRR') AND JV.REF_CODE = DTL.POLICYFINANCEPOSTINGCODE
AND JV.GROUPCODE  IS NULL 
 )M)ACC) WHERE REF_NO NOT IN(SELECT * FROM IM_NOV_REVERSEDOC)  